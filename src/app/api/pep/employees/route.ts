...
export async function GET(req: NextRequest) {
  ...
  try {
    // Simple query without complex JSON aggregation for now
    let query = `SELECT e.*, en.code as entity_code, en.display_name as entity_name FROM employees e JOIN entities en ON en.id = e.entity_id`;
    ...
    // Get portfolios for each employee
    const result = await Promise.all(employees.map(async (emp) => {
      const portfolios = await allAsync<{ id: number; code: string; display_name: string; color: string; allocation_percent: number }>(
        `SELECT p.id, p.code, p.display_name, p.color, ep.allocation_percent 
         FROM employee_portfolios ep JOIN portfolios p ON p.id = ep.portfolio_id 
         WHERE ep.employee_id = $1`,
        [emp.id]
      );
      return { ...emp, portfolios };
    }));
...
export async function POST(req: NextRequest) {
  ...
  if (user.role !== "admin") {
    const permissions = await getUserPermissions(user.id);
    const canEdit = permissions.find(p => p.entityId === entity_id && p.canEdit);
    if (!canEdit) {
  ...
  try {
    ...
    const newEmployee = await getAsync<{ id: number }>("SELECT id FROM employees WHERE entity_id = $1 AND first_name = $2 AND last_name = $3 ORDER BY id DESC LIMIT 1", [entity_id, first_name, last_name]);
...
export async function PUT(req: NextRequest) {
  ...
  const current = await getAsync<{ entity_id: number }>("SELECT entity_id FROM employees WHERE id = $1", [id]);
  if (!current) {
  ...
  if (user.role !== "admin") {
    const permissions = await getUserPermissions(user.id);
    const canEdit = permissions.find(p => p.entityId === current.entity_id && p.canEdit);
    if (!canEdit) {
  ...
  try {
    ...
    if (portfolio_ids !== undefined && user.role === "admin") {
      await execAsync("DELETE FROM employee_portfolios WHERE employee_id = $1", [id]);
...
export async function DELETE(req: NextRequest) {
  ...
  // Get current employee to check entity
  const current = await getAsync<{ entity_id: number }>("SELECT entity_id FROM employees WHERE id = $1", [Number(id)]);
  if (!current) {
    return NextResponse.json({ error: "Employee not found" }, { status: 404 });
  }
  ...
  try {
    await execAsync("DELETE FROM employees WHERE id = $1", [Number(id)]);
